 ...
 *
 * $Id: bcmrobo.c,v 1.1.1.1 2010/03/05 07:31:36 reynolds Exp $
 */
...
#define PAGE_STATUS 0x01    /* Status page, Foxconn added pling 12/04/2006 */
...
bcm_robo_enable_device(robo_info_t *robo)
{
    ...
    /* Foxconn added start pling 01/31/2007 */
#if 0       /* pling removed temp, 04/09/2007 */
    if (nvram_match("qos_enable", "1"))
        bcm_robo_enable_qos(robo, 1);
    else
        bcm_robo_enable_qos(robo, 0);
#endif
    /* Foxconn added end pling 01/31/2007 */
...
int bcm_robo_read_link_status(int *link, int *speed, int *duplex)
{
	...
	robo->ops->read_reg(robo, PAGE_STATUS, REG_LINK_SUM, &val, sizeof(val));
    *link = (val & 0x0F);

	/* Read port speed summary register */
	robo->ops->read_reg(robo, PAGE_STATUS, REG_SPEED_SUM, &val, sizeof(val));
    *speed = (val & 0x0F);

    /* Read duplex status summary register */
	robo->ops->read_reg(robo, PAGE_STATUS, REG_DUPLEX_SUM, &val, sizeof(val));
    *duplex = (val & 0x0F);
...
#if 0       /* pling removed temp, 04/09/2007 */
int bcm_robo_enable_qos(robo_info_t *robo, int enable)
{
    unsigned long long val64;

    if (enable)
    {
        /* Set DSCP priority registers */
        val64 = (3LL << 48) | (3LL << 32) | (2 << 16) | (2 << 0);
	    robo->ops->write_reg(robo, PAGE_QOS, REG_QOS_DSCP_PRIO + 0x8, &val64, sizeof(val64));
    
        val64 = (1LL << 48) | (1 << 0);
	    robo->ops->write_reg(robo, PAGE_QOS, REG_QOS_DSCP_PRIO, &val64, sizeof(val64));

        val64 = 0;
	    robo->ops->read_reg(robo, PAGE_QOS, REG_QOS_DSCP_PRIO, &val64, sizeof(val64));
        printk("DSCP priority register 0x30 = 0x%llx\n", val64);
    
        val64 = 0;
	    robo->ops->read_reg(robo, PAGE_QOS, REG_QOS_DSCP_PRIO+0x8, &val64, sizeof(val64));
        printk("DSCP priority register 0x38 = 0x%llx\n", val64);        
    }
    else
    {
        val64 = 0;
...
#if 0
int bcm_robo_wr_reg(int page, int reg, int val, int size)
{
    robo_info_t *robo = robo_ptr;
    ...
    {
        uint8 u8 = (uint8)val;
        robo->ops->write_reg(robo, page, reg, &u8, size);
    }
    else
    if (size == 2)
    {
        uint16 u16 = (uint16)val;
        robo->ops->write_reg(robo, page, reg, &u16, size);
    }
    else
    if (size == 4)
    {
        uint32 u32 = (uint32)val;
        robo->ops->write_reg(robo, page, reg, &u32, size);
    }
