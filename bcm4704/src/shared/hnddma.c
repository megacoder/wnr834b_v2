 ...
 *
 * $Id: hnddma.c,v 1.1.1.1 2010/03/05 07:31:36 reynolds Exp $
 */
...
_dma_rxfill(dma_info_t *di)
{
	...
	for (i = 0; i < n; i++) {
		...
		*(uint32*)(PKTDATA(di->osh, p)) = 0;
		...
		/* Foxconn added start pling 12/04/2007 */
		/* Adjust for extra 6 bytes from BCM5325F when LAN port QoS is enabled */
		if (dma_qos_enable && di->name[0] == 'e' && di->name[2] == '0')
			((struct sk_buff *)(p))->data += 2;     
		/* Foxconn added end pling 12/04/2007 */
