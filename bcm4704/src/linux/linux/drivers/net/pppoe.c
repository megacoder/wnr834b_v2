...
/* wklin added start, 07/11/2007 */
void clear_pppoe_pch(void)
{
    /* printk("set pch to 0\n"); */
    ppp_ch.pchan = 0;
}
/* wklin added end, 07/11/2007 */
...
struct packet_type pppoed_ptype = {
	...
	type:	__constant_htons(ETH_P_PPPOE_DISC),
	func:	pppoe_disc_rcv,
};
...
static int pppoe_create(struct socket *sock)
{
    ...
    pppoe_sock.sk = sk;
    pppoe_sock.protocol_type = PPP_PROTOCOL_PPPOE;
    ppp_import_sock_info(&pppoe_sock);
...
int pppoe_xmit_wrap(struct sock *sk, struct sk_buff *skb)
{
	return __pppoe_xmit(sk, skb);
}
