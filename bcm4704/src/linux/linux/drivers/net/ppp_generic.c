...
static struct sock_info pppoe_sock;
static unsigned int protocol_type = 0;
...
int __init ppp_init(void)
{
    ...
    memset(&pptp_sock, 0, sizeof(struct sock_info));
    memset(&pppoe_sock, 0, sizeof(struct sock_info));
...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (proto < 0x8000) {
		...
		*skb_push(skb, 2) = 1;
        ...
        ppp_modify_tcp_mss(skb->data, ppp->dev->mtu - 80);
...
ppp_push(struct ppp *ppp)
{
	...
	if ((ppp->flags & SC_MULTILINK) == 0) {
		...
		if (pch->chan) {
            /* PPTP sock would not be NULL, when PPTP was active */
            if ((pptp_sock.sk != NULL) || (pppoe_sock.sk != NULL)) {
                if ((*((unsigned char *)skb->data) <= PPP_NW_LAYER) &&
                    (*((unsigned short *)skb->data) > 0)) {
                    if (protocol_type == PPP_PROTOCOL_PPTP) {
                        pptp_xmit_wrap(pptp_sock.sk, skb);
                    }
                    else if (protocol_type == PPP_PROTOCOL_PPPOE) {
                        pppoe_xmit_wrap(pppoe_sock.sk, skb);
                    }
                    ppp->xmit_pending = NULL;
                }
                else if (pch->chan->ops->start_xmit(pch->chan, skb))
    				ppp->xmit_pending = 0;
            }
            else
...
struct ppp_info
ppp_export_info(void)
{
    return (struct ppp_info)ppp_ch;
}

void
ppp_import_sock_info(struct sock_info *pSockInfo)
{
    protocol_type = pSockInfo->protocol_type;
    if (pSockInfo->protocol_type == PPP_PROTOCOL_PPPOE) {
        pppoe_sock.protocol_type = pSockInfo->protocol_type;
        pppoe_sock.sk = pSockInfo->sk;
    }
    else if (pSockInfo->protocol_type == PPP_PROTOCOL_PPTP) {
        pptp_sock.protocol_type = pSockInfo->protocol_type;
        pptp_sock.sk = pSockInfo->sk;
    }
}
...
EXPORT_SYMBOL(ppp_export_info);
EXPORT_SYMBOL(ppp_import_sock_info);
