 ...
 * This code is GPL
 *
 * $Id: cfi_cmdset_0002.c,v 1.1.1.1 2010/03/05 07:31:35 reynolds Exp $
 *
 */
...
static int do_write_oneword(struct map_info *map, struct flchip *chip, unsigned long adr, __u32 datum, int fast)
{
 ...
 retry:
...
#else	/* !_CFI_RESCHEDULE, Use DQ5 bit as recommended by AMD */
	
	timeo = jiffies + (20 * HZ) / 1000;	/* 20msec upper bound */
	while (((Last[0] = cfi_read(map, adr)) & 0x80) != (datum & 0x80)) {
		if (Last[0] & 0x20) {
			/* Bit DQ5 is set */
			Last[0] = cfi_read(map, adr);
			if ((Last[0] & 0x80) != (datum & 0x80)) {
				printk(KERN_WARNING "Flash Program Device Timeout Error. Device timed out.\n");
				ret = -EIO;
				break;
			}
			cfi_spin_unlock(chip->mutex);
			cfi_udelay(10);
			cfi_spin_lock(chip->mutex);
		} else if (time_after(jiffies, timeo)) {
			printk(KERN_WARNING "Flash Program Device Timeout Error. Timer timed out.\n");
			ret = -EIO;
			break;
		}
	}
	...
	if (ret) {
		printk(KERN_WARNING "Flash Program Device Timeout Error.\n");
		cfi_send_gen_cmd(0xF0, 0, chip->start, map, cfi, cfi->device_type, NULL);
	}
#endif	/* _CFI_RESCHEDULE */
