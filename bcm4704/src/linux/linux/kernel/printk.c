...
int do_syslog2(int type, char * buf, int len)
{
	unsigned long i, j, limit, count;
	int do_clear = 0;
	char c;
	...
	case 2:		/* Read from log */
		...
		if (error)
			goto out;
		error = wait_event_interruptible(log_wait2, (log_start2 - log_end2));
		if (error)
			goto out;
		i = 0;
		spin_lock_irq(&logbuf_lock2);
		while ((log_start2 != log_end2) && i < len) {
...
asmlinkage int printk(const char *fmt, ...)
{
	...
	static int log_to_buf2 = 0;				

#if defined(CONFIG_HWSIM) && defined(mips)
	if (log_buf == real_log_buf)
		log_buf = KSEG1ADDR((char *) real_log_buf);
    
	if (log_buf2 == real_log_buf2)
		log_buf2 = KSEG1ADDR((char *) real_log_buf2);
#endif
...
void release_console_sem(void)
{
	...
	unsigned long must_wake_klogd2 = 0;		

	for ( ; ; ) {
		spin_lock_irqsave(&logbuf_lock, flags);
		must_wake_klogd |= log_start - log_end;
		must_wake_klogd2 |= log_start2 - log_end2;		
