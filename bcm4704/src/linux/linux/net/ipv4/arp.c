 ...
 *
 * Version:	$Id: arp.c,v 1.1.1.1 2010/03/05 07:31:11 reynolds Exp $
 *
...
#define ATTADEV
    ...
    if ((arp->ar_op == htons(ARPOP_REQUEST) || arp->ar_op == htons(ARPOP_REPLY))
       && inet_addr_type(sip)==RTN_UNICAST && memcmp(dev->name,"br0", 3) == 0) {
...
void __init arp_init (void)
{
    ...
    /* wklin added start, 01/08/2007, for attadev */
#ifdef ATTADEV
    {
        static int attadev_init(void);
        attadev_init();
    }
#endif
    /* wklin added end, 01/08/2007 */
...
static int attadev_get_info(char *buffer, char **start, off_t offset, 
        int length);
static int attadev_del_info(char *buffer, char **start, off_t offset, 
        int length);
...
static int attadev_init(void) {
    ...
    /* create /proc/net/attadev for r/w */
	proc_net_create ("attadev", 0, attadev_get_info);
	proc_net_create ("delatta", 0, attadev_del_info);
    return 0;
}

static int attadev_del_info(char *buffer, char **start, off_t offset, int length)
{
    attadev_t *p = &attadevs[0];
    int i=0;
...
static int attadev_get_info(char *buffer, char **start, off_t offset, int length)
{
    attadev_t *p = &attadevs[0];
    int i, size, len=0;
    ...
    for (i=0; i<MAX_ATTADEV_ENTRY; i++) {
        if ((p+i)->flag == FLAG_VALID) {
            size = sprintf(buffer+len, "%08X %02X:%02X:%02X:%02X:%02X:%02X\n", 
                    ntohl((p+i)->sip),
