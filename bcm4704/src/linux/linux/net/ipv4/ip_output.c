 ...
 *
 * Version:	$Id: ip_output.c,v 1.1.1.1 2010/03/05 07:31:11 reynolds Exp $
 *
...
#ifndef CONFIG_ACOSNAT_L26
#include <linux/brlock.h>                           //no this file in linux 2.6
#endif
...
//We need to use this function in acosnatfw.c
...
int ip_finish_output2(struct sk_buff *skb)
{
    ...
    #ifdef CONFIG_ACOSNAT_L26                             //2.6 kernel
    rcu_read_lock();
    spin_lock_bh(&agnat_hook_lock);                    
    #else                                               //2.4 kernel
    br_read_lock_bh(BR_NETPROTO_LOCK);
    #endif
    
    if (NULL != acosL7Filter_hook_printf)
        (*acosL7Filter_hook_printf)(skb, 1);
    
    #ifdef CONFIG_ACOSNAT_L26                             //2.6 kernel
    spin_unlock_bh(&agnat_hook_lock);
    rcu_read_unlock();
    #else                                               //2.4 kernel
...
__inline__ int ip_finish_output(struct sk_buff *skb)
{
	...
	//	       ip_finish_output2);
       return ACOSNAT_HOOK(PF_INET, ACOSNAT_IP_POST_ROUTING, skb, NULL, dev,
		       ip_finish_output2);
}
