 ...
 * $Id: etc47xx.c,v 1.1.1.1 2010/03/05 07:31:06 reynolds Exp $
 */
...
static void chipinit(ch_t *ch, uint options); /* BRCM patch, 06/15/2007 */
...
chipattach(etc_info_t *etc, void *osh, void *regsva)
{
	...
	/* get sb handle */
	if ((ch->sbh = sb_attach(etc->deviceid, ch->osh, regsva, PCI_BUS, NULL, &ch->vars,
	                         &ch->vars_size)) == NULL) {
	 ...
	 * the ethernet.
	 */
	if ((regs = (bcmenetregs_t*)sb_setcore(ch->sbh, SB_ENET, etc->unit)) == NULL) {
	...
	/* configure pci core */
	sb_pci_setup(ch->sbh, (1 << sb_coreidx(ch->sbh)));
	...
	if ((ch->di = dma_attach(osh, name, ch->sbh,
	...
	if ((boardtype == BCM94710D_BOARD) ||
		...
		sb_gpioout(ch->sbh, 0x44, 0, GPIO_DRV_PRIORITY);
		sb_gpioouten(ch->sbh, 0x44, 0x44, GPIO_DRV_PRIORITY);
		/* Hold phys in reset for a nice long 2 ms */
		for (i = 0; i < 2; i++)
			OSL_DELAY(1000);
		sb_gpioout(ch->sbh, 0x44, 0x44, GPIO_DRV_PRIORITY);
		sb_gpioouten(ch->sbh, 0x44, 0, GPIO_DRV_PRIORITY);
	}
	...
	if (((boardflags & BFL_ENETROBO) ||
		...
		if (!(etc->robo = bcm_robo_attach(ch->sbh, ch, ch->vars,
...
chipinit(struct bcm4xxx *ch, uint options) /*BRCM patch, 06/15/2007*/
{
	etc_info_t *etc;
	bcmenetregs_t *regs;
	uint idx;
	...
	/* BRCM patch, 06/15/2007 */
	 if (options & ET_INIT_FULL) {
		...
		if (options & ET_INIT_INTRON) /*BRCM patch, 06/15/2007*/
		et_intrson(etc->et);
	}
	else
		dma_rxenable(ch->di);
...
chipstatsupd(struct bcm4xxx *ch)
{
	 ...
	 */
	etc->mib.tx_broadcast_pkts += R_REG(ch->osh, &regs->mib.tx_broadcast_pkts);
	etc->mib.tx_multicast_pkts += R_REG(ch->osh, &regs->mib.tx_multicast_pkts);
	etc->mib.tx_len_64 += R_REG(ch->osh, &regs->mib.tx_len_64);
	etc->mib.tx_len_65_to_127 += R_REG(ch->osh, &regs->mib.tx_len_65_to_127);
	etc->mib.tx_len_128_to_255 += R_REG(ch->osh, &regs->mib.tx_len_128_to_255);
	etc->mib.tx_len_256_to_511 += R_REG(ch->osh, &regs->mib.tx_len_256_to_511);
	etc->mib.tx_len_512_to_1023 += R_REG(ch->osh, &regs->mib.tx_len_512_to_1023);
	etc->mib.tx_len_1024_to_max += R_REG(ch->osh, &regs->mib.tx_len_1024_to_max);
	etc->mib.tx_jabber_pkts += R_REG(ch->osh, &regs->mib.tx_jabber_pkts);
	etc->mib.tx_oversize_pkts += R_REG(ch->osh, &regs->mib.tx_oversize_pkts);
	etc->mib.tx_fragment_pkts += R_REG(ch->osh, &regs->mib.tx_fragment_pkts);
	etc->mib.tx_underruns += R_REG(ch->osh, &regs->mib.tx_underruns);
	etc->mib.tx_total_cols += R_REG(ch->osh, &regs->mib.tx_total_cols);
	etc->mib.tx_single_cols += R_REG(ch->osh, &regs->mib.tx_single_cols);
	etc->mib.tx_multiple_cols += R_REG(ch->osh, &regs->mib.tx_multiple_cols);
	etc->mib.tx_excessive_cols += R_REG(ch->osh, &regs->mib.tx_excessive_cols);
	etc->mib.tx_late_cols += R_REG(ch->osh, &regs->mib.tx_late_cols);
	etc->mib.tx_defered += R_REG(ch->osh, &regs->mib.tx_defered);
	etc->mib.tx_carrier_lost += R_REG(ch->osh, &regs->mib.tx_carrier_lost);
	etc->mib.tx_pause_pkts += R_REG(ch->osh, &regs->mib.tx_pause_pkts);
	etc->mib.rx_broadcast_pkts += R_REG(ch->osh, &regs->mib.rx_broadcast_pkts);
	etc->mib.rx_multicast_pkts += R_REG(ch->osh, &regs->mib.rx_multicast_pkts);
	etc->mib.rx_len_64 += R_REG(ch->osh, &regs->mib.rx_len_64);
	etc->mib.rx_len_65_to_127 += R_REG(ch->osh, &regs->mib.rx_len_65_to_127);
	etc->mib.rx_len_128_to_255 += R_REG(ch->osh, &regs->mib.rx_len_128_to_255);
	etc->mib.rx_len_256_to_511 += R_REG(ch->osh, &regs->mib.rx_len_256_to_511);
	etc->mib.rx_len_512_to_1023 += R_REG(ch->osh, &regs->mib.rx_len_512_to_1023);
	etc->mib.rx_len_1024_to_max += R_REG(ch->osh, &regs->mib.rx_len_1024_to_max);
	etc->mib.rx_jabber_pkts += R_REG(ch->osh, &regs->mib.rx_jabber_pkts);
	etc->mib.rx_oversize_pkts += R_REG(ch->osh, &regs->mib.rx_oversize_pkts);
	etc->mib.rx_fragment_pkts += R_REG(ch->osh, &regs->mib.rx_fragment_pkts);
	etc->mib.rx_missed_pkts += R_REG(ch->osh, &regs->mib.rx_missed_pkts);
	etc->mib.rx_crc_align_errs += R_REG(ch->osh, &regs->mib.rx_crc_align_errs);
	etc->mib.rx_undersize += R_REG(ch->osh, &regs->mib.rx_undersize);
	etc->mib.rx_crc_errs += R_REG(ch->osh, &regs->mib.rx_crc_errs);
	etc->mib.rx_align_errs += R_REG(ch->osh, &regs->mib.rx_align_errs);
	etc->mib.rx_symbol_errs += R_REG(ch->osh, &regs->mib.rx_symbol_errs);
	etc->mib.rx_pause_pkts += R_REG(ch->osh, &regs->mib.rx_pause_pkts);
	etc->mib.rx_nonpause_pkts += R_REG(ch->osh, &regs->mib.rx_nonpause_pkts);
