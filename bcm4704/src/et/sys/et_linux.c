 ...
 *
 * $Id: et_linux.c,v 1.1.1.1 2010/03/05 07:31:06 reynolds Exp $
 */
...
void et_init(et_info_t *et, uint options); /* BRCM patch, 06/15/2007 */
...
/* Foxconn added start pling 08/03/2006 */
#include <linux/proc_fs.h>
#if defined (CONFIG_PROC_FS) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2,4,0))
static int et_proc_net_status(char *buffer, char **start, off_t offset, int length, int *eof, void *data);
static int et_proc_wan_status(char *buffer, char **start, off_t offset, int length, int *eof, void *data);
#endif
/* Foxconn added end pling 08/03/2006 */

/* Foxconn added start pling 01/30/2007 */
#ifdef INCLUDE_QOS
#include "et_qos.c"
#endif
/* Foxconn added end pling 01/30/2007 */
...
/* Foxconn added start pling 12/12/2007 */
#ifdef INCLUDE_WAN_DEBUG
#include <linux/ip.h>
...
static char packet_buf[8192];
static char packet_buf_temp[1024];
...
static void dump_pkt(struct sk_buff *skb)
{
    ...
    unsigned int packet_len, payload_len;
    int i;

    packet_len = skb->len;
    
    /* System time mark */
    sprintf(packet_buf, "%lu", xtime.tv_sec);

    /* Print ethernet header */
    eth_hdr = (struct ethhdr *)&(skb->data[0]);
    sprintf(packet_buf_temp, ", DA:");
    strcat(packet_buf, packet_buf_temp);
    for (i=0; i<6; i++)
    {
        sprintf(packet_buf_temp, " %02X", (unsigned char)(eth_hdr->h_dest[i]));
        strcat(packet_buf, packet_buf_temp);
    }
    sprintf(packet_buf_temp, ", SA:");
    strcat(packet_buf, packet_buf_temp);
    for (i=0; i<6; i++)
    {
        sprintf(packet_buf_temp, " %02X", (unsigned char)(eth_hdr->h_source[i]));
        strcat(packet_buf, packet_buf_temp);
    }
    ether_type = ntohs(eth_hdr->h_proto);
    sprintf(packet_buf_temp, ", type: 0x%04X, ", ether_type);
    strcat(packet_buf, packet_buf_temp);

    payload = (unsigned char *)&eth_hdr[1];
    payload_len = packet_len - sizeof(struct ethhdr);

    /* Print packet type, and optionally the header */
    switch(ether_type)
    {
        case ETH_P_IP:
            iph = (struct iphdr *)payload;
            iph_len = (iph->ihl) * 4;
            sprintf(packet_buf_temp, "IP, Src: %d.%d.%d.%d, Dst: %d.%d.%d.%d, header:",
                    (unsigned char)((iph->saddr & 0x000000FF)),
                    (unsigned char)((iph->saddr & 0x0000FF00) >> 8),
                    (unsigned char)((iph->saddr & 0x00FF0000) >> 16),
                    (unsigned char)((iph->saddr & 0xFF000000) >> 24),
                    (unsigned char)((iph->daddr & 0x000000FF)),
                    (unsigned char)((iph->daddr & 0x0000FF00) >> 8),
                    (unsigned char)((iph->daddr & 0x00FF0000) >> 16),
                    (unsigned char)((iph->daddr & 0xFF000000) >> 24));
            strcat(packet_buf, packet_buf_temp);
            for (i=0; i<iph_len; i++)
            {
                sprintf(packet_buf_temp, " %02X", (unsigned char)payload[i]);
                strcat(packet_buf, packet_buf_temp);
            }
            sprintf(packet_buf_temp, ", payload:");
            strcat(packet_buf, packet_buf_temp);

            payload = &payload[iph_len];
            payload_len -= iph_len;
            
            break;

        case ETH_P_ARP:
            sprintf(packet_buf_temp, "ARP:");
            strcat(packet_buf, packet_buf_temp);
            break;

        case ETH_P_PPP_DISC:
            sprintf(packet_buf_temp, "PPPoE,");
            strcat(packet_buf, packet_buf_temp);
            if (payload[1] == CODE_PADI)
                sprintf(packet_buf_temp, " PADI,");
            else
            if (payload[1] == CODE_PADO)
                sprintf(packet_buf_temp, " PADO,");
            else
            if (payload[1] == CODE_PADR)
                sprintf(packet_buf_temp, " PADR,");
            else
            if (payload[1] == CODE_PADS)
                sprintf(packet_buf_temp, " PADS,");
            else
            if (payload[1] == CODE_PADT)
                sprintf(packet_buf_temp, " PADT,");
            else
                strcpy(packet_buf_temp, "");
            strcat(packet_buf, packet_buf_temp);
        ...
        case ETH_P_PPP_SES:
            sprintf(packet_buf_temp, "PPPoE, header:");
            strcat(packet_buf, packet_buf_temp);

            for (i=0; i<PPPOE_HDR_LEN; i++)
            {
                sprintf(packet_buf_temp, " %02X", (unsigned char)payload[i]);
                strcat(packet_buf, packet_buf_temp);
            }
            ...
            payload = &payload[PPPOE_HDR_LEN];
            payload_len -= PPPOE_HDR_LEN;
            break;

        default:
            sprintf(packet_buf_temp, "unknown,");
            strcat(packet_buf, packet_buf_temp);
            break;
    }
...
#endif
/* Foxconn added end pling 12/12/2007 */
...
et_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
    ...
    /* Foxconn added start pling 08/03/2006 */
	/* Create /proc/net entry to show link status */
#if defined (CONFIG_PROC_FS) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2,4,0))
    if (et->etc->unit == 1) {
    	create_proc_read_entry("net/wan_status", 0, 0, et_proc_wan_status, (void*)et);
        create_proc_read_entry("net/net_status", 0, 0, et_proc_net_status, (void*)et);
    }
#endif
    /* Foxconn added end pling 08/03/2006 */
...
/* Foxconn added start pling 08/03/2006 */
#if defined (CONFIG_PROC_FS) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2,4,0))
extern int bcm_robo_read_link_status(int *link, int *speed, int *duplex);
...
static int et_proc_net_status(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
    ...
    bcm_robo_read_link_status(&link, &speed, &duplex);
    ...
    if (link) {
		...
		sprintf(line1, "Link Down");

    /* Read WAN link status */
    if (et->etc->linkstate) {
		sprintf(line2, "%dM/%s",
			et->etc->speed,
			et->etc->duplex ? "Full" : "Half");
	} else
		sprintf(line2, "Link Down");

	sprintf(buffer, "LAN %s\nWAN %s\n", line1, line2);
...
et_sendnext(et_info_t *et)
{
	...
	while (etc->pioactive == NULL) {
        ...
        /* Foxconn added start pling 01/30/2007 */
#ifdef INCLUDE_QOS
        if (qos_enable && qos_do_add_strip_tag && etc->unit == 0)   /* LAN only */
            skb = et_tx_add_bcm_tag(et, &skb);
#endif
        /* Foxconn added end pling 01/30/2007 */
...
void
et_init(et_info_t *et, uint options) /* BRCM patch, 06/15/2007 */
/*et_init(et_info_t *et)*/
{
	...
	/*etc_init(et->etc);*/
	etc_init(et->etc, options); /*BRCM patch, 06/15/2007*/
}
...
et_set_multicast_list(struct net_device *dev)
{
	...
	if (etc->up) {
		...
		/*et_init(et);*/
		et_init(et, ET_INIT_DEF_OPTIONS); /*BRCM patch, 06/15/2007*/
	}
...
et_dpc(ulong data)
{
	...
	if (et->events & INTR_ERROR)
		if ((*chops->errors)(ch))
		    et_init(et, ET_INIT_INTROFF); /* BRCM patch, 06/15/2007 */
			/*et_init(et);*/
...
et_sendup(et_info_t *et, struct sk_buff *skb)
{
    ...
    /* Foxconn added start pling 02/05/2007 */
#ifdef INCLUDE_QOS
    if (qos_enable && et->etc->unit == 0)
        et_rx_strip_bcm_tag(et, skb);
#endif
    /* Foxconn added end pling 02/05/2007 */
