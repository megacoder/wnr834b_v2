 ...
 *
 * $Id: emfc.c,v 1.1.1.1 2010/03/05 07:31:37 reynolds Exp $
 */
#include <bcmdefs.h>
#include <bcmendian.h>
                        ...
                        uint32 dest_ip, bool rt_port)
{
	...
	if (!rt_port)
	{
		...
		emf_sendup(emfc->emfi, sdu_clone);
	...
	for (ptr = emfc->iflist_head; ptr != NULL; ptr = ptr->next)
	{
		...
		if (emf_forward(emfc->emfi, sdu_clone, dest_ip,
...
emfc_input(emfc_info_t *emfc, void *sdu, void *ifp, uint8 *iph, bool rt_port)
{
	...
	else
	{
		     ...
		     ptr != mgrp->mi_head.prev; ptr = ptr->next)
		{
			...
			emf_forward(emfc->emfi, sdu_clone,
	        ...
	        if (ifp != mi->mi_mhif->mhif_ifp)
		{
			...
			emf_forward(emfc->emfi, sdu, dest_ip,
	...
	switch (cfg->oper_type)
	{
		...
		case EMFCFG_OPER_TYPE_SET:
			...
			if (emfc->emf_enable)
			{
				...
				if (emf_hooks_register(emfc->emfi) != SUCCESS)
				{
					cfg->status = EMFCFG_STATUS_FAILURE;
					cfg->size = sprintf(cfg->arg,
			...
			else
			{
				...
				emf_hooks_unregister(emfc->emfi);
 ...
 */
emfc_info_t *
emfc_init(int8 *inst_id, void *emfi, osl_t *osh)
{
	emfc_info_t *emfc;
