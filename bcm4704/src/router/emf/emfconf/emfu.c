 ...
 *
 * $Id: emfu.c,v 1.1.1.1 2010/03/05 07:31:37 reynolds Exp $
 */
#include <stdio.h>
#include <sys/types.h>
#include <typedefs.h>
...
emf_cfg_iface_add(char *argv[])
{
	...
	strncpy(cfg_if->if_name, argv[4], 16);
	cfg_if->if_name[15] = 0;
...
emf_cfg_uffp_add(char *argv[])
{
	...
	if (strncmp(argv[2], "uffp", 4) == 0)
	{
		...
		strncpy(cfg_uffp->if_name, argv[4], 16);
	...
	else
	{
		...
		strncpy(cfg_rtport->if_name, argv[4], 16);
...
emf_cfg_mfdb_add(char *argv[])
{
	...
	strncpy(mfdb_entry->if_name, argv[5], 16);
