 ...
 *
 * $Id: defaults.c,v 1.1.1.1 2010/03/05 07:31:40 reynolds Exp $
 */
...
struct nvram_tuple router_defaults[] = {
	...
	/* Radar parameters by phy for 20 and 40 MHz channels respectively 
	 * (npulses ncontig min_pw max_pw thresh0 thresh1 blank fmdemodcfg npulses_lp min_pw_lp
	 *  max_pw_lp min_fm_lp max_deltat_lp min_deltat max_deltat
	 */
	{ "wl_radarargs_11a",
	"5 4 4 100 0x2b0 0x2f0 0x0 0x0 8 500 4000 10 240000000 1000 3000000", 0 },
	{ "wl_radarargs_11n",
	"5 4 3 100 0x6b0 0x6f0 0x2c40 0x7f09 6 300 1200 25 200000000 1000 3000000", 0},
	{ "wl_radarargs40_11n",
	"5 4 4 100 0x6b0 0x6d9 0x2c40 0x7f07 12 195 2000 25 400000000 1000 6000000", 0 },
...
fix_name(const char *name, char *fixed_name)
{
	...
	if (name[0] == 'w' && name[1] == 'l' && (name[2] >= '0' && name[2] <= '9')) {
		src_offset = (name[3] == '.') ? 5 : 3;
		dst_offset = 2;  /* Writing in wl here */
		strcpy(fixed_name, "wl");
	}

	strcpy(&fixed_name[dst_offset], &name[src_offset]);
}
