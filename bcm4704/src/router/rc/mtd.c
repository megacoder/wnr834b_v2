 ...
 *
 * $Id: mtd.c,v 1.1.1.1 2010/03/05 07:31:38 reynolds Exp $
 */
...
mtd_write(const char *path, const char *mtd)
{
	...
	FILE *fp;
	char *buf = NULL;
	long count, write_count, len, off;
	int ret = -1;
	...
	for (erase_info.start = 0; erase_info.start < trx.len; erase_info.start += count) {
		...
		/* Update CRC */
		crc = hndcrc32(&buf[off], count - off, crc);
		...
		(void) ioctl(mtd_fd, MEMUNLOCK, &erase_info);
		if (ioctl(mtd_fd, MEMERASE, &erase_info) != 0) {
			fprintf(stderr, "MEMERASE failed\n");
			perror(mtd);
			goto fail;
		}

		if ((write_count = write(mtd_fd, buf, count)) != count) {
			fprintf(stderr, "mtd: write failed (Tried to write %ld, wrote %ld)\n",
			        count, write_count);
			goto fail;
		}
	}
