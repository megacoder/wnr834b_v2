 ...
 *
 * $Id: network.c,v 1.1.1.1 2010/03/05 07:31:38 reynolds Exp $
 */
...
wl_vif_hwaddr_set(const char *name)
{
	...
	ether_atoe(ea, ifr.ifr_hwaddr.sa_data);
...
unload_emf(void)
{
	...
	if (query_module("emf", QM_INFO, &info, sizeof(info), &ret) == 0) {
		eval("rmmod", "igs");
		eval("rmmod", "emf");
	}
...
#ifdef __CONFIG_EMF__
			if ((i == 0) && nvram_match("emf_enable", "1")) {
