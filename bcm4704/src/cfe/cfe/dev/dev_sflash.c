 ...
 *
 * $Id: dev_sflash.c,v 1.1.1.1 2010/03/05 07:31:40 reynolds Exp $
 */
...
sflash_cfe_read(cfe_devctx_t *ctx, iocb_buffer_t *buffer)
{
	...
	while (len) {
		if ((bytes = sflash_read(sflash->sbh, sflash->cc, offset, len, buf)) < 0) {
			ret = bytes;
			goto done;
		}
		offset += bytes;
...
sflash_cfe_write(cfe_devctx_t *ctx, iocb_buffer_t *buffer)
{
	...
	while (len) {
		...
		/* Erase block */
		if ((ret = sflash_erase(sflash->sbh, sflash->cc, (uint) cur.buf_offset)) < 0)
			goto done;
		while (sflash_poll(sflash->sbh, sflash->cc, (uint) cur.buf_offset));

		/* Write holding block */
		while (cur.buf_length) {
			if ((bytes = sflash_write(sflash->sbh, sflash->cc,
						  (uint) cur.buf_offset,
						  (uint) cur.buf_length,
						  (uchar *) cur.buf_ptr)) < 0) {
				ret = bytes;
		 ...
		 void *probe_ptr)
{
	...
	sflash->sbh = sb_kattach(SB_OSH);

	sflash->cc = (chipcregs_t *)probe->flash_cmdset;

	/* Initialize serial flash access */
	if (!(sflash->info = sflash_init(sflash->sbh, sflash->cc))) {
